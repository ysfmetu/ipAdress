...
public class ServerController {
    ...
    @PostMapping("/pasif/{id}")
    public ResponseEntity<Response> pasifServer(@PathVariable("id") Long id) throws IOException {

        return ResponseEntity.ok(
                Response.builder()
                        .timeStamp(now())
                        .data(Map.of("server",serverService.pasif(id)))
                        .message("server kaydı pasife çekilmiştir")
                        .status(HttpStatus.OK)
                        .statusCode(HttpStatus.OK.value())
                        .build()
        );
